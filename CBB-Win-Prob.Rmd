...
collapse <- subset(final_pbp, win_prob >.85 & winner == '0' & game_play_number>100) %>%
              dplyr::select(c(home_team_name, away_team_name,score_diff, win_prob, game_play_number,
                              end_game_seconds_remaining,game_id)) %>%
              filter(away_team_name %in% c('Virginia Tech','Virginia','Wake Forest','Vanderbilt',
                                    'UConn','Texas Tech','South Carolina','Ohio State','Ole Miss',
                                    'Michigan','Louisville','Georgia Tech','Iowa','Florida',
                                    'Arizona State'))
...
```{r plots}
### Plotting!
ggplot(example, aes(x = game_play_number,
                     y = 1-win_prob,
                     color = chance_win,
                     group = 1)) +
  geom_line(size=.8) + 
